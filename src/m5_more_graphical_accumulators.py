...
def draw_squares_from_circle(n, circle, window):
    ...
    for x in range(n):
        ...
        window.render(.05)
        start_point=rg.Point(start_point.x+radius,start_point.y+radius)
        end_point=rg.Point(end_point.x+radius,end_point.y+radius)
...
def run_test_draw_circles_from_rectangle():
    ...
    rectangle1= rg.Rectangle(rg.Point(150,150),rg.Point(175,175))
    draw_circles_from_rectangle(5,8,rectangle1,window1)

    rectangle11 =rg.Rectangle(rg.Point(450,450),rg.Point(495,495))
    draw_circles_from_rectangle(10,15,rectangle11,window1)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range (m):
        center = rg.Point((center_Rectangle.x-width/2-height*(1/2+k)),
                          center_Rectangle.y)
        circle= rg.Circle(center,height/2)
        circle.attach_to(window)
        window.render(.05)
    for k in range(n):
        center = rg.Point((center_Rectangle.x),
                          center_Rectangle.y-height/2-width*(1/2+k))
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    center_rectangle1=rectangle1.get_center()
    center_rectangle2= rectangle2.get_center()

    for k in range (n):
        delta_x = k*(width1/2)
        delta_y= k*(height/2)
        start = rg.Point(center_rectangle1.x-delta_x,
                         center_rectangle1.y+delta_y)
        end = rg.Point(center_rectangle2.x-delta_x,center_rectangle2.y+delta_y)
